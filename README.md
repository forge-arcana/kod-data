...
https://cdn.jsdelivr.net/gh/forge-arcana/kod-data@main/econ/v1/latest.min.json    # relative economic multiplier
```
...
**econ** — per-country relative economic multiplier, USD-anchored to the US = 1.0.

```json
{ "dataset": "econ", "base": "USD", "unit": "multiplier", "date": "2026-07-13",
  "reference": "US", "multiplierFloor": 0.05,
  "countries": { "US": { "multiplier": 1, "incomeGroup": "High income", "gniPerCapitaPppUsd": 89490, "dataYear": 2025, "basis": "gni_ppp" },
                 "IN": { "multiplier": 0.13, "incomeGroup": "Lower middle income", "gniPerCapitaPppUsd": 11600, "dataYear": 2025, "basis": "gni_ppp" } } }
```

`multiplier = clamp(GNI_pc_PPP(country) / GNI_pc_PPP(US), multiplierFloor)`. Multiply a base
price by it (`localPrice ≈ base × multiplier`), or bucket `multiplier` / `incomeGroup` into
your own tiers. A classified country with no GNI datum gets `basis: "income_group"` (multiplier
imputed from its income group's median). It is a normaliser, not a currency — `econ` is not part
of the USD cross-rate math below.
...
| econ | `worldbank-income`, `worldbank-gni` | [World Bank Open Data](https://data.worldbank.org/) — income-group classification + GNI per capita, PPP |
...
scope for a daily reference API. The market domains (fx, metals, crypto, rates,
yields) refresh on that daily cadence; `econ` is annual — the World Bank's
income-group classification refreshes each July and GNI per capita PPP lags by
roughly a year, so its `date` moves once a year, not once a day.
