...
const TOOLS = [
  ...
  },
  {
    name: 'get_multiplier',
    description:
      'Relative economic multiplier per country (World Bank, USD-anchored to US = 1.0): an affordability / PPP ' +
      'normaliser to multiply a base price by (localPrice ≈ base × multiplier), plus income group and GNI per ' +
      'capita PPP. Optionally filter to specific ISO-3166 alpha-2 country codes.',
    inputSchema: {
      type: 'object',
      properties: { countries: { type: 'array', items: { type: 'string' }, description: 'ISO-3166 alpha-2 codes, e.g. ["US","IN"]. Omit for all.' } },
      additionalProperties: false,
    },
    run: async ({ countries } = {}) => {
      const e = await getJson('econ/v1/latest.min.json')
      const all = e.countries || {}
      let out = all
      if (countries && countries.length) {
        const want = new Set(countries.map((s) => String(s).toUpperCase()))
        out = Object.fromEntries(Object.entries(all).filter(([k]) => want.has(k.toUpperCase())))
      }
      return { unit: e.unit, date: e.date, updated: e.updated, reference: e.reference, multiplierFloor: e.multiplierFloor, incomeGroups: e.incomeGroups, sources: e.sources, countries: out }
    },
  },
]
  ...
  switch (method) {
    case 'initialize':
      return ok(id, {
        ...
        instructions:
          'Keppet Open Data: free USD-anchored FX, metals, crypto, rates, yields and per-country economic multipliers. ' +
